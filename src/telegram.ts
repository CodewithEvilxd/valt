...
export async function sendFileToChannel(fileBuffer: Buffer, filename: string, mimetype: string): Promise<TelegramFileInfo> {
  ...
  console.log('TELEGRAM API RESPONSE:', JSON.stringify(res.data, null, 2));
  if (!res.data.ok) {
    console.error('Telegram API error details:', res.data);
  ...
  let fileObj = msg.document || msg.video || msg.audio || msg.animation;
